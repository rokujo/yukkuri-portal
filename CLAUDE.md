# 上松ゆっくり塾 学習アプリ窓口サイト 仕様書

## 1. プロジェクト概要

上松ゆっくり塾（個別指導・少人数）が制作した学習アプリ群を、塾外の利用者・塾生の双方に提供する窓口サイト。アメブロで運用中の塾ブログから誘導し、各アプリへのアクセスを一元化する。
...
- **一般のブログ読者**：アメブロから流入、興味があればカンパで利用。
...
```json
{
  ...
  "site": {
    "name": "上松ゆっくり塾",
    "tagline": "上松ゆっくり塾の、ありそうでなかった学習アプリ",
    "description": "現場の指導から生まれた学習アプリ群。AIを使いながら、生徒のペースに寄り添います。",
    "blogUrl": "https://ameblo.jp/yukkuri-jyuku/",
    "kampaInfo": {
      "halfYear": 2000,
      "year": 3500,
      "currency": "JPY",
      "paymentMethods": ["銀行振込", "手渡し"],
      "studentsFree": true,
...
ブログ記事の末尾に「アプリ一覧はこちら」のリンクを貼る運用。記事ごとに案内文を統一しておくと管理が楽。
...
- カンパ申し込みが月10件を超えたら、Stripe等の決済自動化を検討

### 10.3 不要な機能（最初は実装しない）

- ユーザー登録・ログイン（共通パスワードで十分）
- アプリの利用統計・ダッシュボード（必要になってから）
- お問い合わせフォーム（アメブロ経由でいい）
- 多言語対応（日本語のみ）
...
- 塾名：**上松ゆっくり塾**
- アメブロURL：**https://ameblo.jp/yukkuri-jyuku/**
- キャッチコピー：**上松ゆっくり塾の、ありそうでなかった学習アプリ**
